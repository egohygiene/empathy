...
export function createReactViteConfig(options: ReactViteConfigOptions) {
  ...
  return defineConfig(({ command, mode }) => {
    ...
    const configuredBasePath = options.basePathEnvironmentVariable
      ? environment[options.basePathEnvironmentVariable]
      : environment.VITE_BASE_PATH;
    const basePath = normalizeBasePath(configuredBasePath ?? options.basePath ?? "/");
    const analyze = readBoolean(environment.TEMPLATE_ANALYZE);
    const sourceMaps = readBoolean(environment.TEMPLATE_SOURCE_MAPS, !production);
    ...
    if (options.pwa) {
      plugins.push(
        VitePWA({
          ...
          devOptions: {
            enabled: readBoolean(environment.TEMPLATE_PWA_DEV),
          },
        }),
      );
    }
    ...
    return {
      ...
      server: {
        host: environment.VITE_DEV_HOST ?? "127.0.0.1",
        port: readPort(environment.VITE_DEV_PORT, options.serverPort ?? 5173),
        strictPort: true,
      ...
      preview: {
        host: environment.VITE_PREVIEW_HOST ?? "127.0.0.1",
        port: readPort(environment.VITE_PREVIEW_PORT, options.previewPort ?? 4173),
        strictPort: true,
      },
      build: {
        target: "es2022",
