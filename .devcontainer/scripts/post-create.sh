...
# @description
#   Runs inside a newly created development container after workspace content
#   has been reconciled. This script provides repository-agnostic orchestration;
#   project-specific setup belongs in the extension hooks listed below.
#
#   Responsibilities:
...
#   - execute repository-specific post-create hooks in deterministic order
#   - establish a container-aware completion marker
#
#   Non-responsibilities:
#   - choosing a package manager automatically
#   - installing dependencies without an explicit repository hook
#   - updating Git submodules without an explicit repository hook
#   - running database migrations without an explicit repository hook
#   - starting long-running services
#
# Extension points, executed in this order:
#   .devcontainer/scripts/post-create.local.sh
#   .devcontainer/scripts/post-create.d/*.sh
#
# Suggested ordered hooks (create only those the repository needs):
#   .devcontainer/scripts/post-create.d/10-update-submodules.sh
#   .devcontainer/scripts/post-create.d/20-install-node-dependencies.sh
#   .devcontainer/scripts/post-create.d/30-install-python-dependencies.sh
#   .devcontainer/scripts/post-create.d/40-install-playwright.sh
#   .devcontainer/scripts/post-create.d/50-run-migrations.sh
#
...
#   DEVCONTAINER_POST_CREATE_CONTINUE_ON_HOOK_FAILURE=1
#     Warn and continue when an extension hook fails. The default is fail-fast.
#   DEVCONTAINER_POST_CREATE_FORCE=1
#     Run setup even when a matching completion marker exists.
#   DEVCONTAINER_POST_CREATE_VERBOSE=1
#     Enable verbose diagnostic logging.
#   DEVCONTAINER_POST_CREATE_TRACE=1
#     Enable Bash execution tracing. This may expose command arguments.
#
# @exitcode 0 Successful execution.
# @exitcode 1 Post-create setup failed.
...
readonly STATUS_PREFIX="🛠️  [devcontainer:post-create]"
readonly WARNING_PREFIX="⚠️  [devcontainer:post-create]"
readonly ERROR_PREFIX="❌ [devcontainer:post-create]"
readonly DEBUG_PREFIX="🔎 [devcontainer:post-create]"

readonly DEVCONTAINER_DIRECTORY=".devcontainer"
readonly POST_CREATE_STATE_DIRECTORY_NAME="devcontainer"
readonly POST_CREATE_MARKER_NAME="post-create.complete"
readonly POST_CREATE_SCHEMA_VERSION="1"
...
REPO_ROOT=""
SCRIPT_DIRECTORY=""
POST_CREATE_STATE_DIRECTORY=""
POST_CREATE_MARKER_FILE=""
TEMPORARY_FILE=""
...
on_exit() {
    ...
    if ((exit_code != 0 && ERROR_REPORTED == 0)); then
        print_warning "Post-create lifecycle stopped with exit code $exit_code."
    fi
...
on_signal() {
    ...
    ERROR_REPORTED=1
    print_warning "Received $signal_name; stopping the post-create lifecycle."
    exit "$exit_code"
}
...
print_usage() {
    ...
    printf "\n"
    printf "Run repository-agnostic Dev Container post-create orchestration.\n"
    printf "\n"
    printf "Options:\n"
    ...
    printf "  --force               Ignore a matching post-create marker and run again.\n"
...
configure_environment() {
    : "${HOME:?HOME must be set for the Dev Container post-create lifecycle}"
    ...
    POST_CREATE_STATE_DIRECTORY="$XDG_STATE_HOME/$POST_CREATE_STATE_DIRECTORY_NAME"
    POST_CREATE_MARKER_FILE="$POST_CREATE_STATE_DIRECTORY/$POST_CREATE_MARKER_NAME"

    export DEVCONTAINER_ID
    export DEVCONTAINER_RUNTIME_ID="$RUNTIME_CONTAINER_ID"
    export DEVCONTAINER_LIFECYCLE="post-create"
...
create_required_directories() {
    ...
    ensure_directory "$POST_CREATE_STATE_DIRECTORY" 0700
...
is_already_completed() {
    [[ -f $POST_CREATE_MARKER_FILE ]] || return 1

    grep -Fqx "schema_version=$POST_CREATE_SCHEMA_VERSION" \
        "$POST_CREATE_MARKER_FILE" || return 1
...
##
# Atomically writes the post-create completion marker.
##
write_post_create_marker() {
    local completed_at

    if ((DRY_RUN == 1)); then
        print_status "Would write post-create marker: $POST_CREATE_MARKER_FILE"
        return 0
    fi

    completed_at="$(date -u "+%Y-%m-%dT%H:%M:%SZ")"
    TEMPORARY_FILE="$(mktemp "${POST_CREATE_MARKER_FILE}.tmp.XXXXXX")"

    {
        printf "schema_version=%s\n" "$POST_CREATE_SCHEMA_VERSION"
    ...
    mv "$TEMPORARY_FILE" "$POST_CREATE_MARKER_FILE"
    TEMPORARY_FILE=""

    print_status "Wrote post-create marker: $POST_CREATE_MARKER_FILE"
}
...
##
# Executes one repository-specific post-create hook.
#
# Arguments:
#   $1 - Hook path.
##
run_hook() {
    ...
    if is_enabled "${DEVCONTAINER_POST_CREATE_CONTINUE_ON_HOOK_FAILURE:-0}"; then
...
##
# Discovers and executes post-create hooks in deterministic filename order.
##
run_extension_hooks() {
    local hooks_directory="$REPO_ROOT/$DEVCONTAINER_DIRECTORY/scripts/post-create.d"
    local local_hook="$REPO_ROOT/$DEVCONTAINER_DIRECTORY/scripts/post-create.local.sh"
    local hook
...
##
# Runs the container-side Dev Container post-create lifecycle.
#
# Arguments:
#   $@ - Script arguments.
##
main() {
    ...
    if is_enabled "${DEVCONTAINER_POST_CREATE_VERBOSE:-0}"; then
        VERBOSE=1
    fi

    if is_enabled "${DEVCONTAINER_POST_CREATE_FORCE:-0}"; then
        FORCE_POST_CREATE=1
    fi

    if is_enabled "${DEVCONTAINER_POST_CREATE_TRACE:-0}"; then
        set -o xtrace
    fi
    ...
    print_status "Running post-create setup for the development container."
    ...
    if ((FORCE_POST_CREATE == 0)) && is_already_completed; then
        print_status "Development container post-create state is already complete."
        return 0
    fi
    ...
    print_status "Development container post-create lifecycle complete."
}
