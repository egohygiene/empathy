...
def common_summary(
    *,
    ...
    return {
        ...
        "freshness": {
            "expires_at": format_timestamp(generated_at + timedelta(days=stale_after_days)),
...
def normalize_osv(document: dict[str, Any], summary: dict[str, Any]) -> None:
    ...
    counts = {name: int(severity.get(name, 0)) for name in SEVERITY_ORDER}
...
def normalize_megalinter(
    ...
    for linter in active_linters:
        ...
        error_count = int(linter.get("total_number_errors") or linter.get("number_errors") or 0)
        warning_count = int(linter.get("total_number_warnings") or 0)
...
def scorecard_sarif_checks(document: dict[str, Any]) -> tuple[list[dict[str, Any]], int]:
    ...
    for run in document["runs"]:
        ...
        if isinstance(run_results, list):
            results.extend(result for result in run_results if isinstance(result, dict))
...
def matched_scorecard_api(
    ...
    if repo.get("commit") != commit:
        return "stale", None
    if not isinstance(api_document.get("score"), (int, float)) or not isinstance(
        api_document.get("checks"), list
    ):
        return "invalid", None
...
def normalize_scorecard(
    ...
    if matched_api is not None:
        aggregate_score = float(matched_api["score"])
        api_checks = []
        for check in matched_api["checks"]:
            if not isinstance(check, dict) or not isinstance(check.get("score"), (int, float)):
                continue
